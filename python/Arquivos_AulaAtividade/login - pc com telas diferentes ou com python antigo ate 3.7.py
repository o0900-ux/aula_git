...
class TelaPerfil(BoxLayout):
    def __init__(self, email, **kwargs):
        ...
        self.add_widget(AsyncImage(source='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRrO0ac2OB9y0a7Pkcd48pvjwIANFLrdHYCdTgDkmj_marY-jzojyimNTJ49YqctWxEIg&usqp=CAU'))
        
        self.add_widget(Label(text=f"Bem-vindo, {email}.\nVocê estar usando, versão beta do App!\nTela de Verificação de usuario =).", color=(0, 0, 0.2, 1), bold=True, font_size=16))
        
        botao_layout = BoxLayout(padding=8)
        self.add_widget(botao_layout)
        
        self.deslogar = Button(text="Deslogar", background_color=get_color_from_hex('#ff0000'), size_hint=(None, None), width=270, height=50, )
...
class Telalogin(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.add_widget(Label(text="Insira o seu E-mail", color=(0, 0, 0.2, 1), bold=True, font_size=16, size_hint_y=None, height=40))
        self.Email = TextInput(hint_text="Digite o Seu E-mail", multiline=False, width=310, size_hint=(None, None), height=40)
        self.add_widget(self.Email)
        
        self.add_widget(Label(text="Insira sua Senha:", color=(0, 0, 0.2, 1), bold=True, font_size=16, size_hint_y=None, height=40))
        self.Senha = TextInput(hint_text="Digite sua senha", password=True, multiline=False, width=310, size_hint=(None, None), height=40)
        self.add_widget(self.Senha)
        
        botao_layout = BoxLayout(padding=8, size_hint_y=None, height=100)
        self.add_widget(botao_layout)
        
        self.enviar = Button(text="Cadastra", background_color=get_color_from_hex('#3498db'), size_hint=(None, None), width=145, height=50, )
        self.enviar.bind(on_press=self.Cadastra)
        botao_layout.add_widget(self.enviar)
        
        self.login = Button(text="login", background_color=get_color_from_hex('#ff0000'), size_hint=(None, None), width=145, height=50, )
